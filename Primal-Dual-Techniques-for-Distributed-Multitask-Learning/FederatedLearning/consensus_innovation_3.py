...
def consensus_innovations_algorithm(num_iterations, datapoints, adj_matrix, step_size=0.1, num_nodes=200, calculate_score=False):
    ...
    for i in range(num_iterations):
        ...
        for k in range(num_nodes):
            for l in range(num_nodes):
                consensus_term = np.dot(adj_matrix[k, l], weights) / np.sum(adj_matrix[k, :])
...
def get_consensus_innovation_MSE(K, datapoints, samplingset, matrix):
    total_error, _ = consensus_innovations_algorithm(K, datapoints, matrix, calculate_score=True)
...
print(max(total_values[0]))
print(min(total_values[0]))