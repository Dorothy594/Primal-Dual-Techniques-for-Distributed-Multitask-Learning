...
#         w_ci[:, k, 0] = samplingset[k]['w']  # 假设 datapoints 已经包含了初始权重
#
#     for i in range(1, K):
#         # 使用 SAGA 更新每个节点
#         for k in range(network_size):
...
#             w_ci[:, k, i] = saga(samplingset[k]['f_grad'], w_ci[:, k, i-1], n_iter, step_size, n, samplingset)
#             w_ci[:, k, i] *= prob[k, k]
...
def fun(penalty_func, matrix):
    samplingset_indices = random.sample([j for j in range(200)], k=int(0.8 * 200))
    samplingset = [datapoints[idx] for idx in samplingset_indices]  # 从datapoints中获取采样数据点
...
plt.semilogy(x_total[0:10], np.mean(total_values, axis=(0, 1))[0:10], label='total')
plt.title('Train - Frist 10 Iteration')
plt.show()
plt.close()