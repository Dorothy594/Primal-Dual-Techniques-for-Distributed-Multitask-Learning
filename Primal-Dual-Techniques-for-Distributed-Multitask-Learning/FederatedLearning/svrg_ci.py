...
def svrg_consensus_innovation(iterations, datapoints, adj_matrix, learning_rate=0.01, lambda_reg=0,
                              calculate_score=False, m=10):
    num_nodes = adj_matrix.shape[1]  # Number of nodes
    feature_size = datapoints[0]['features'].shape[1]
    weights = np.zeros((num_nodes, feature_size))  # Initialize local weights
    iteration_scores = []

    # Compute full gradient at the start
    full_gradient = np.zeros((num_nodes, feature_size))
    for i in range(num_nodes):
        prediction = np.dot(datapoints[i]['features'], weights[i])
        error = prediction - datapoints[i]['label']
        full_gradient[i] = np.dot(datapoints[i]['features'].T, error) + lambda_reg * weights[i]

    for outer_iter in range(iterations):
        weights_new = np.copy(weights)

        # Inner loop: variance-reduced updates
        for inner_iter in range(m):
            for i in range(num_nodes):
                ...
                idx = np.random.randint(datapoints[i]['features'].shape[0])
                X_i = datapoints[i]['features'][idx:idx + 1]
                y_i = datapoints[i]['label'][idx:idx + 1]
                gradient = np.dot(X_i.T, (np.dot(X_i, weights[i]) - y_i)) + lambda_reg * weights[i]

                # Variance-reduced gradient
                gradient -= np.dot(X_i.T, (np.dot(X_i, weights[i]) - y_i)) - full_gradient[i]
...
def get_consensus_innovation_MSE(K, datapoints, samplingset, matrix):
    total_error, _ = svrg_consensus_innovation(K, datapoints, matrix, calculate_score=True)
