...
def gradient_of_local_objective(w, data):
    Q = np.eye(len(w))  # Identity matrix as a placeholder for Hessian matrix
    b = np.ones(len(w))  # Ones vector as a placeholder for linear term
    return Q @ w - b  # Adjusted to be a descent gradient


# def consensus_innovation(num_iterations, datapoints, adj_matrix, step_size=0.01, num_nodes=200, calculate_score=False):
#     n = next(iter(datapoints.values()))['features'].shape[1]
#     weights = np.random.randn(num_nodes, n)
#     iteration_scores = []
#
#     # A = np.ones((100, 100))
...
#             # consensus_term = np.dot(adj_matrix[k, :], weights) / np.sum(adj_matrix[k, :])
#             consensus_term = np.dot(adj_matrix[k, :], weights)
#             innovation_term = gradient_of_local_objective(weights[k], datapoints[k]['features'])
#             new_weights[k] = consensus_term - step_size * innovation_term
#             # new_weights[k] *= prob[k]
#
#         weights = new_weights
#
#         if calculate_score:
...
# def get_consensus_innovation_MSE(K, datapoints, samplingset, not_samplingset, new_matrix):
#     new_matrix = np.array(new_matrix)
#     total_error, _ = consensus_innovation(K, datapoints, new_matrix)
#     train_error, _ = consensus_innovation(K, samplingset, new_matrix)
...
def get_scores(datapoints, predicted_w, fl_w, samplingset, others=True):
    ...
    consensus_innovation_score = None
    ...
    if others:
        ...
        # consensus_innovation_score = get_consensus_innovation_MSE(K, datapoints, reformated_samplingset, reformated_not_samplingset, new_matrix)
...
def fun(penalty_func):
    ...
    # _, ci_w = consensus_innovation(K, samplingset, new_matrix)
