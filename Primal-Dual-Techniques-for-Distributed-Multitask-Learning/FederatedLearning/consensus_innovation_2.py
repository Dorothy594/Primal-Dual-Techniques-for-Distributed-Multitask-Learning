...
def consensus_innovation(K, samplingset, new_matrix):
    w_ci = np.random.randn(M, network_size, K)
    ...
    for k in range(network_size):
        for l in range(network_size):
            ...
            w_ci[:, k, 1] *= prob[l, k]
    for k in range(network_size):
        w_ci[:, k, 1] += - np.true_divide(mu, len(samplingset)) * rho * w_ci[:, k, 0] + np.true_divide(mu, len(samplingset)) * (d_k[:, k] - np.dot(H_k[:, :, k].T, w_ci[:, k, 0]))
        w_ci[:, k, 1] *= prob[k, k]
    ...
    for i in range(2, K):
        ...
        for k in range(network_size):
            for l in range(network_size):
                ...
                w_ci[:, k, i] *= prob[l, k]
        for k in range(network_size):
            w_ci[:, k, i] += - np.true_divide(mu, len(samplingset)) * rho * w_ci[:, k, i-1] + np.true_divide(mu, len(samplingset)) * (d_k[:, k] - np.dot(H_k[:, :, k].T, w_ci[:, k, i-1]))
            w_ci[:, k, i] *= prob[k, k]
