...
def get_scores(K, datapoints, samplingset):
    N = len(datapoints)
    '''
    ...
    # ciMSE.append(consensus_innovation_score)

    return consensus_innovation_score
...
def fun(penalty_func):
    ...
    return penalty_func, get_scores(K, datapoints, samplingset)
...
print('consensus + innovation:',
      '\n mean train MSE:', np.mean(train_values[:100]),
      '\n mean test MSE:', np.mean(test_values[:100]))

# 生成横坐标
# x = np.arange(len(consensus_innovation_scores))
# x_total = np.arange(len(total_values[4][199]))
x_train = np.arange(len(train_values[0][0][:100]))
x_test = np.arange(len(test_values[0][0][:100]))

plt.plot(x_train, train_values[0][0][:100], label='train')
plt.savefig('train_100.png')
plt.plot(x_train, test_values[0][0][:100], label='test')
plt.savefig('test_100.png')


# 绘制折线图
# plt.plot(x_total, total_values[4][199], label='total')
# print("Total:")
# print(total_values[4][199])
# plt.savefig('result_total.png')
# plt.plot(x_train, train_values[4][79], label='train')
# print("Train:")
# print(train_values[4][79])
# plt.savefig('result_train.png')
# plt.plot(x_test, test_values[4][119], label='test')
# print("Test:")
# print(test_values[4][119])
