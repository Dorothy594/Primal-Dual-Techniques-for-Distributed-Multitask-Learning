...
def consensus_innovation(K, samplingset):
    # w_ci = np.zeros((M, 100, K))
    # error_ci = np.zeros((100, K))
    w_ci = np.zeros((M, len(samplingset), K))
    ...
    for i in range(1, K):
        ...
        for k in range(len(samplingset)):
            ...
            # error_ci[k, i] = np.square(np.linalg.norm(w_ci[:, k, i] - y[samplingset]))
    return error_ci, w_ci


def extra(K, samplingset):
    # w_extra = np.zeros((M, 100, K))
    # psi_extra = np.zeros((M, 100, K))
    # error_extra = np.zeros((100, K))
    w_extra = np.zeros((M, len(samplingset), K))
    psi_extra = np.zeros((M, len(samplingset), K))
    error_extra = np.zeros((len(samplingset), K))
    for i in range(1, K):
        for k in range(len(samplingset)):
            for l in range(len(samplingset)):
                psi_extra[:, k, i] += B[l, k] * w_extra[:, l, i - 1]

        for k in range(len(samplingset)):
            w_extra[:, k, i] += psi_extra[:, k, i]
            psi_extra[:, k, i] += -np.true_divide(mu, N) * rho * w_extra[:, k, i - 1] + np.true_divide(mu, N) * (
                        d_k[:, k] - np.dot(H_k[:, :, k].T, w_extra[:, k, i - 1]))
            w_extra[:, k, i] += psi_extra[:, k, i] - psi_extra[:, k, i - 1]
            error_extra[k, i] = np.square(np.linalg.norm(w_extra[:, k, i] - w_star))
    return w_extra, psi_extra, error_extra
...
def get_extra_MSE(K, datapoints, samplingset):
    not_samplingset = [i for i in range(len(datapoints)) if i not in samplingset]
    _, _, total_error = extra(K, datapoints)
    _, _, train_error = extra(K, samplingset)
    _, _, test_error = extra(K, not_samplingset)
    extra_MSE = {'total': total_error,'train': train_error, 'test': test_error}
    return extra_MSE


def get_scores(K, datapoints, predicted_w, fl_w, samplingset, others=True):
    N = len(datapoints)
    '''
    ...
    consensus_innovation_score = get_consensus_innovation_MSE(K, datapoints, samplingset)
    # extra_score = get_extra_MSE(K, datapoints, samplingset)
    ...
    return alg_1_score, fl_score, linear_regression_score, decision_tree_score, consensus_innovation_score
...
w_tracking = np.zeros((M, 100, K))

g_tracking = np.zeros((M, 100, K))

error_tracking = np.zeros((100, K))
...
def fun(penalty_func):
    ...
    # _, w_ci_new = consensus_innovation(K, samplingset)

    return penalty_func, get_scores(K, datapoints, predicted_w, fl_w, samplingset, penalty_func=='norm1')
...
for penalty_func, scores in results:
    alg1_score, fl_score, linear_regression_score, decision_tree_score, consensus_innovation_score = scores
    ...
    # extra_scores[penalty_func].append(extra_score)

labels = ['alg1,n1', 'alg1,n2', 'alg1,mocha', 'FedAvg', 'lin_reg', 'd_tree', 'con+inn']
...
# print('EXTRA:',
#       '\n mean train MSE:', np.mean([item['train'] for item in extra_scores['norm1']]),
#       '\n mean test MSE:', np.mean([item['test'] for item in extra_scores['norm1']]))
...
consensus_innovation_score = [item['total'] for item in consensus_innovation_scores['norm1']]
# extra_score = [item['total'] for item in extra_scores['norm1']]
...
mean_MSEs = [
    ...
    np.mean(consensus_innovation_score)
    # np.mean(extra_score)
]
...
std_MSEs = [
    ...
    np.std(consensus_innovation_score)
    # np.std(extra_score)
]
